...
export async function createServiceOrder(formData: FormData) {
  try {
    ...
    const currentRate = type === 'INTERNAL' ? assembler.internalRate : assembler.externalRate;
    ...
    const commissionValue = new Prisma.Decimal(Number(saleValue) * (currentRate / 100));
